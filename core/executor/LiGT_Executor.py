...
from core.model import LiGT
from core.data import LiGT_VQADataset, textlayout_ocr_adapt
...
class LiGT_Executor(Base_Executor):
        ...
        with torch.no_grad():
            for it, batch in enumerate(dataloader):
                ...
                pred = self.model.generate(input_ids = batch['input_ids'].to(self.config.DEVICE),
                                            hashed_2d_l1 = batch['2D_L1'].to(self.config.DEVICE),
                                            hashed_2d_l2 = batch['2D_L2'].to(self.config.DEVICE),
                                            hashed_2d_l3 = batch['2D_L3'].to(self.config.DEVICE),
                                            hashed_2d_l4 = batch['2D_L4'].to(self.config.DEVICE),
    ...
    def _create_data_utils(self):
        ...
        train_qa_df = pd.read_csv(self.config.qa_train_path)[["image_id", "questionId", "question", "answer", "image"]]
        val_qa_df = pd.read_csv(self.config.qa_val_path)[["image_id", "questionId", "question", "answer", "image"]]
        ...
        self.train_data = LiGT_VQADataset(
                                        qa_df = train_qa_df,
                                        ocr_df = ocr_df,
    ...
    def _init_eval_predict_mode(self):
        ...
        if self.mode == "eval":
            ...
            predict_qa_df = pd.read_csv(self.config.qa_predict_path)[["image_id", "questionId", "question", "answer", "image"]]
        
            ocr_df = textlayout_ocr_adapt(self.config.ocr_path, h_scale=1, w_scale=1)

            self.predict_data = LiGT_VQADataset(
                                                qa_df = predict_qa_df,
                                                ocr_df = ocr_df,
                                                tokenizer = self.tokenizer,
                                                max_input_length = self.config.max_input_length,
