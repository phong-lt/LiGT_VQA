...
class ExHashed2D_VQADataset(Dataset):
    def __init__(self,
                 qa_df,
                 ocr_df,
                 tokenizer,
        ...
        self.feature = ["input_ids", "attention_mask", "token_type_ids", "image_id", "question_id", "2D_L1", "2D_L2", "2D_L3", "2D_L4"]
        self.data = dict()
    ...
    def create_features(self, ques, words, bounding_box):
        ...
        if max_input_length > self.max_length:
            max_ocr_ids = len(ocr_ids) - max_input_length + self.max_length
        ...
        ques_hashed_ids = self.tokenizer([self.alphabet2d[0]]*len(ques_ids), is_split_into_words=True,
                                        add_special_tokens=False)['input_ids']
        ...
        if max_input_length > self.max_length:
            ...
            hashed_2d_l1_ids = [self.tokenizer.pad_token_id] + ques_hashed_ids + [self.tokenizer.eos_token_id]*2 \
              + hashed_2d_l1_according_to_ocr_ids[:len(ocr_ids) - max_input_length + self.max_length] + [self.tokenizer.eos_token_id]

            hashed_2d_l2_ids = [self.tokenizer.pad_token_id] + ques_hashed_ids  + [self.tokenizer.eos_token_id]*2 \
              + hashed_2d_l2_according_to_ocr_ids[:len(ocr_ids) - max_input_length + self.max_length] + [self.tokenizer.eos_token_id]
            
            hashed_2d_l3_ids = [self.tokenizer.pad_token_id] + ques_hashed_ids  + [self.tokenizer.eos_token_id]*2 \
              + hashed_2d_l3_according_to_ocr_ids[:len(ocr_ids) - max_input_length + self.max_length] + [self.tokenizer.eos_token_id]
            
            hashed_2d_l4_ids = [self.tokenizer.pad_token_id] + ques_hashed_ids  + [self.tokenizer.eos_token_id]*2 \
              + hashed_2d_l4_according_to_ocr_ids[:len(ocr_ids) - max_input_length + self.max_length] + [self.tokenizer.eos_token_id]
    ...
    def get_start_end_index(self):
        ...
        for index in range(len(answers)):
            ...
            if i + 1 == 1 or (i + 1) % 5000 == 0 or i+1 == len(answers):
                log.info(f"Indexing... {i+1}/{len(answers)}")
